...
public class JavaStreamTask1 {
  public static void main(String[] args) {
    ...
    System.out.print("Task1. Выбрать все матчи которые были в первой половине месяца: ");
    List<Match> matches1 = matches.stream()
            .filter(e -> e.getDate().get(Calendar.DAY_OF_MONTH) < e.getDate().getActualMaximum(Calendar.DAY_OF_MONTH) / 2)
            .toList();
    System.out.printf("%d records\n", matches1.size());
    System.out.println(matches1);

    System.out.print("Task2. Посчитать общее количество матчей в выборке: ");
    System.out.printf("%d records\n", matches.size());

    System.out.print("Task3. Выбрать топ 10 самых посещаемых матчей: ");
    List<Match> matches3 = matches.stream()
            .sorted((o1, o2) -> (-1) * (o1.getWatchers() - o2.getWatchers()))
            .limit(10)
            .toList();
    System.out.printf("%d records\n", matches3.size());
    System.out.println(matches3);

    System.out.print("Task4. Выбрать топ 3 матчей с самой большой разницей в счете на стадионе WWK Arena: ");
    List<Match> matches4 = matches.stream()
            .filter(e -> e.getStadium() == "WWK Arena")
            .sorted(Comparator.comparingInt(o -> (-1) * (Math.abs(o.getResult1() - o.getResult2()))))
            .limit(3)
            .collect(Collectors.toList());
    System.out.printf("%d records\n", matches4.size());
    System.out.println(matches4);

    System.out.print("Task5. Посчитать сколько матчей содержат упоминания о стадионе: ");
    long countWithStadium = matches.stream()
            .filter(e -> e.getStadium().equals("NULL"))
            .count();
    System.out.printf("%d records\n", countWithStadium);

    System.out.println("Task6.1 Посчитать сколько матчей сыграла каждая команда в качестве гостя (в формате map): ");
    System.out.println("- без использования Stream и посчитать время работы метода -");
    long timeStart1 = System.currentTimeMillis();
    Map<String, Integer> teamPlayGuestByOrdinal = new HashMap<>();
    for (Match m: matches) {
      if (!teamPlayGuestByOrdinal.containsKey(m.getTeam2())){
        teamPlayGuestByOrdinal.put(m.getTeam2(), 1);
      } else {
        teamPlayGuestByOrdinal.put(m.getTeam2(), teamPlayGuestByOrdinal.get(m.getTeam2()) + 1);
      }
    }
    System.out.println("TIME: " + (System.currentTimeMillis() - timeStart1));
    System.out.println(teamPlayGuestByOrdinal);

    System.out.println("Task6.2 Посчитать сколько матчей сыграла каждая команда в качестве гостя (в формате map): ");
    System.out.println("- и с помощью stream API и посчитать время работы метода -");
    long timeStart2 = System.currentTimeMillis();
    Map<String, Integer> teamPlayGuestByStream = matches.stream()
            .collect(Collectors.toMap(Match::getTeam2, e -> 1, Integer::sum))
            ;
    System.out.println("TIME: " + (System.currentTimeMillis() - timeStart2));
    System.out.println(teamPlayGuestByStream);

    System.out.print("Task7. выделить наиболее популярный день проведения матча: ");
    Calendar dayPopular = matches.stream()
            .collect(Collectors.toMap(Match::getDate, e -> 1, Integer::sum))
            .entrySet()
            .stream()
            .sorted(Map.Entry.<Calendar, Integer>comparingByValue().reversed())
            .findFirst()
            .get()
            .getKey();
    System.out.println(dayPopular.get(Calendar.YEAR) + "-" + dayPopular.get(Calendar.MONTH) + "-" + dayPopular.get(Calendar.DATE));
  }
}
